...
#region Gaze Visualization
	...
	public void StartVisualizingGaze ()
	{
        ...
        if ( !PupilMarker.TryToReset(_markerLeftEye) )
			_markerLeftEye= new PupilMarker("LeftEye_2D",PupilSettings.leftEyeColor,true);
		if ( !PupilMarker.TryToReset(_markerRightEye) )
			_markerRightEye = new PupilMarker("RightEye_2D",PupilSettings.rightEyeColor,true);
		if ( !PupilMarker.TryToReset(_markerGazeCenter) )
			_markerGazeCenter = new PupilMarker("Gaze_2D",Color.red);
        if (!PupilMarker.TryToReset(_markerCalibrated)) {
            _markerCalibrated = new PupilMarker("Gaze_C", Color.green);
        }
    ...
    public void hideGaze() { PupilMarker.TryToSetActive(_markerGazeCenter, false); }
    public void showGaze() { PupilMarker.TryToSetActive(_markerGazeCenter, true); }
	...
	void VisualizeGaze ()
	{
		if (PupilTools.IsGazing)
		{
			if (PupilTools.CalibrationMode == Calibration.Mode._2D)
			{
                ...
                _markerCalibrated.UpdateCalibratePosition(PupilData._2D.GazePosition);
            }
